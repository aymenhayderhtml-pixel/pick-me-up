...
public class CombatManager : MonoBehaviour
{
    ...
    IEnumerator BattleLoop()
    {
        ...
        while (!IsBattleOver())
        {
            ...
            foreach (var attacker in turnOrder)
            {
                ...
                // Pick target
                CombatUnit target = PickTarget(attacker);
    ...
    IEnumerator HandleDeath(CombatUnit unit)
    {
        ...
        if (unit.isHero)
        {
            ...
            // Morale hit on surviving heroes
            foreach (var hero in _heroes.Where(h => h.IsAlive))
            {
                int moraleHit = -10;
                if (hero.trait == PersonalityTrait.Compassionate) moraleHit = -20;
                if (hero.trait == PersonalityTrait.Stoic)         moraleHit = 0;
                hero.heroRef?.ModifyMorale(moraleHit);
            }
