...
public class BattleUI : MonoBehaviour
{
    ...
    void ShowResults(BattleResult result)
    {
        ...
        if (txt_Fallen != null)
        {
            string fallenNames = result.fallen.Count > 0
                ? string.Join(", ", result.fallen.ConvertAll(h => h.DisplayName))
                : "None";
            txt_Fallen.text = result.fallen.Count > 0
                ? $"Fallen: {fallenNames}"
                : "No casualties.";
            txt_Fallen.color = result.fallen.Count > 0 ? Color.red : Color.white;
        }
